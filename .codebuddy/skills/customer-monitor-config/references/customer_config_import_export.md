...
6. **客户配置导入规则**：
   - 申报数据异常：模板两列（城市名称、业务类型），去重 key 为 `addrName_businessType`，同城市+业务类型只插一条
   - Ukey异常：模板两列（城市名称、业务类型），导出时每城市按社保/公积金拆成两行，导入时同城市多行聚合为一条记录（`socialMonitor`/`fundMonitor` 各自独立），去重 key 为 `addrCode`（同城市只能有一条记录）